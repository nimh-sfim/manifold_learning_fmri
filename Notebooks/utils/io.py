...
def load_LE_SI(sbj_list,check_availability=False, verbose=False, wls=45, wss=1.5, ms=le_ms,dist_metrics=le_dist_metrics,knns=le_knns,norm_methods=norm_methods,input_datas=input_datas, no_tqdm=False):
    ...
    num_missing_files = 0
    num_needed_files = 0
    for m in tqdm_notebook(ms, desc='Final Dimensions',disable=no_tqdm):
        for dist in tqdm_notebook(dist_metrics, desc='Distance Metric',leave=False,disable=no_tqdm):
            for knn in knns:
    ...
    if verbose:
       print('++ INFO [load_LE_SI]: Number of files missing: [%d/%d] files' % (num_missing_files,num_needed_files))
    return si_LE
...
def load_TSNE_SI(sbj_list,check_availability=False, verbose=False, wls=45, wss=1.5,
    ...
    return si_TSNE   
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP # def load_LE_SI(sbj_list,check_availability=False, verbose=False, wls=45, wss=1.5, ms=le_ms,dist_metrics=le_dist_metrics,knns=le_knns,norm_methods=norm_methods,input_datas=input_datas, no_tqdm=False):
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #     """Loads Silhouette Index results for all available scans reduced via Laplacian Eigenmaps
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #     
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        Inputs:
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        -------
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        sbj_list: List of subjects. Add 'ALL' to also load group-level results
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        check_availability: if True, nothing is read from disk. It only check if files exists. Useful to check if all jobs have completed successfully.
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        verbose: True means show more info about progress
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        wls: window length in seconds [Default = 45 secs]
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        wss: window spte in seconds [Default = 1.5 secs]
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        ms: list of low dimensions. [Default = les_ms]
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        dist_metrics: list of distance functions [Default = le_dist_metrics]
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        knns: list of neighhorhood sizes [Default = le_knns]
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        norm_methods: list of normalization schemes [Default = norm_methods]
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        input_datas: list of input data types [Default = input_datas]
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        Returns:
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        --------
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #        si_LE: dataframe with one row per scan and the following columns: 'Subject','Norm','Metric','Knn','m','Target','SI'
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #               for individual scans, target should only include Window Name
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #               for group-level, target should include both Subject and Window Name
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #     """
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #     si_LE = pd.DataFrame(columns=['Subject','Input Data','Norm','Metric','Knn','m','Target','SI'])
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #     num_missing_files = 0
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #     num_needed_files = 0
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #     for m in tqdm_notebook(ms, desc='Final Dimensions',disable=no_tqdm):
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #         for dist in tqdm_notebook(dist_metrics, desc='Distance Metric',leave=False,disable=no_tqdm):
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #             for knn in knns:
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                 for norm_method in norm_methods:
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                     for sbj in sbj_list:
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                         for input_data in input_datas:
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                            num_needed_files +=1
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                            path = osp.join(PRJ_DIR,'Data_Interim','PNAS2015',sbj,'LE',input_data,'{sbj}_Craddock_0200.WL{wls}s.WS{wss}s.LE_{dist}_k{knn}_m{m}.{norm_method}.SI.pkl'.format(norm_method=norm_method,sbj=sbj,
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                                                                                                                                          wls=str(int(wls)).zfill(3), 
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                                                                                                                                          wss=str(wss),
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                                                                                                                                          dist=dist,
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                                                                                                                                          knn=str(knn).zfill(4),
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                                                                                                                                          m=str(m).zfill(4)))
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                            if not osp.exists(path):
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                               num_missing_files +=1
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                               if verbose:
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                                   print('-e "%s"' % path, end=' ')
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                               continue
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                            if not check_availability:
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                               aux = pd.read_pickle(path)
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                               label_ids = list(aux.index)
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                               for label in label_ids:
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                                  si_LE = si_LE.append({'Subject':sbj,'Input Data':input_data,
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                                                        'Norm':norm_method,'Metric':dist,
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                                                        'Knn':knn,'m':m,'Target':label.split('_')[1],'SI':aux[label]}, ignore_index=True)
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #                               
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #     print('++ INFO: Number of files missing: [%d/%d] files' % (num_missing_files,num_needed_files))
# PREVIOUS VERSION - PRE MULTI-M RESULTS FOR GROUP #     return si_LE
...
def load_UMAP_SI(sbj_list,check_availability=False, verbose=False, wls=45, wss=1.5, 
                 ...
                 knns=umap_knns,ms=umap_ms, alphas=umap_alphas, min_dist=0.8, no_tqdm=False):
